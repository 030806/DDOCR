...
- 局部识别采用独立任务：上传文件可直接框选原图区域创建区域 OCR Job；已完成或部分成功任务可以复用 `file_id` 派生新的区域 Job，并用 `source_job_id` 保留追溯关系。来源任务与派生任务不共享 OCR Result，派生操作不改变来源任务状态、审核、纠正或留言。区域范围固化为原图像素 `[x1,y1,x2,y2]`，Worker 通过 `OcrRegion` 执行 ROI 推理，输出继续写入标准页面和结果表。
